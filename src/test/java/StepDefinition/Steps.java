...
//	String server1 = "https://gorest.co.in";
//	RequestSpecification req_spec1 = null;
//	Response resp1 = null;
//	String token = "748b6d7e9c8ed5900d9942cafc44a5b208b35990ca53792be17b20d9f27e809b";

	/*
	 * String server = "https://gorest.co.in"; RequestSpecification req_spec = null;
	 * Response resp = null;
	 */
	 ...
	 * 
	 * // ------Adding logs--------------- logger =
	 * Logger.getLogger("Cucumber_demo");
	 * PropertyConfigurator.configure("log4j.properties");
	 * 
	 * // ----Reading Properties file configprop = new Properties(); //
	 * --------Opening file in input mode FileInputStream fis = new
	 * FileInputStream("config.properties"); configprop.load(fis);
	 * 
	 * 
	 * String br = configprop.getProperty("browser"); if (br.equals("chrome")) {
	 * System.setProperty("webdriver.chrome.driver",
	 * configprop.getProperty("chromepath")); driver = new ChromeDriver();
	 * 
...
//		logger.info("*********** LAUNCHING BROWSER**********");
	...
	/*
	 * @Given("Go Rest API is up and running") public void
	 * go_rest_api_is_up_and_running() {
	 * 
	 * req_spec = given().baseUri(server); // req_spec.header("Authorization",
	 * "Bearer" ,
	 * "748b6d7e9c8ed5900d9942cafc44a5b208b35990ca53792be17b20d9f27e809b"); }
	 * 
	 * @When("I hit the api with get request and end point as {string}") public void
	 * i_hit_the_api_with_get_request_and_end_point_as(String endpoint) { resp =
	 * req_spec.when().get(endpoint); String str = resp.asString();
	 * System.out.println("----------------Response is------------ " + str); }
	 * 
	 * @Then("API should return all the users") public void
	 * api_should_return_all_the_users() {
	 * 
	 * resp.then().assertThat().statusCode(200).body("code", equalTo(200)) //
	 * .body("data[0].name", equalTo("Aamod Gandhi")) // .body("data[0].id",
	 * equalTo("3139")) .body("data", not(emptyArray()));
	 * 
	 * }
	 * 
	 * @Then("API should return user details of user id {string}") public void
	 * api_should_return_user_details_of_user_id(String userid) {
	 * resp.then().assertThat().statusCode(200).body("data.id", equalTo(userid)); }
	 */
	// =========++++++++++++++++++++++==============================
	...
	public void api_is_up_and_running() {
		...
		req_spec1 = given().baseUri(server1).header("Authorization", "Bearer 748b6d7e9c8ed5900d9942cafc44a5b208b35990ca53792be17b20d9f27e809b");

	}

	@When("Hit API as get request and end point as {string}")
	public void hit_api_as_get_request_and_end_point_as(String endpoint) {

		System.out.println("------------------print 11-----------------");
		resp1 = req_spec1.when().get(endpoint);
	...
	public void api_should_return_list_of_the_users() {
		System.out.println("-------------------print 12-----------------");

		resp1.then().assertThat().statusCode(200)
	//	.body("[0].email", equalTo("agarwal_anilabh@zieme-gulgowski.biz"))
				.body("data", not(emptyArray()));
	}

	@Then("API should return details of single user")
	public void api_should_return_details_of_single_user() {
		resp1.then().assertThat().statusCode(200);
		//.body("email", equalTo("agarwal_anilabh@zieme-gulgowski.biz"));
	}
